...
@Component({
  ...
  styleUrl: './supplier-plans.component.css'
})
export class SupplierPlansComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (user && user.id) {
      ...
      if (fromRegistration === 'true') {
        console.log('👋 User just registered - will show welcome message with payment');
  ...
  public selectPlanForPayment(planId: 'basic' | 'corporate'): void {
    console.log('📋 User selected plan for payment:', planId);
    ...
    console.log('✅ Plan selected, ready for payment');
  }
  ...
  public proceedToPayment(): void {
    console.log('💳 Proceeding to payment with plan:', this.selectedPlanForPayment);
  ...
  private updatePlanSelectionDisplay(): void {
    this.plans.forEach(plan => {
      // Marcar el plan seleccionado para pago
  ...
  updatePlansDisplay(): void {
    // Solo usar la lógica original si NO está en modo de selección
  ...
  private async updatePlanAfterSuccessfulPayment(): Promise<void> {
    ...
    console.log('🔄 Activando suscripción después del pago exitoso');
    console.log('   UserId:', userId);
    console.log('   Selected Plan:', this.selectedPlanForPayment);
    ...
    this.configurationService.activateSubscription(userId, this.selectedPlanForPayment).subscribe({
      next: (updatedConfig) => {
        console.log('✅ Suscripción activada exitosamente:', updatedConfig);
        console.log('   Plan:', updatedConfig.subscriptionPlan);
        console.log('   Status:', updatedConfig.subscriptionStatus);
        ...
        // *** CAMBIO IMPORTANTE: Redirigir al dashboard de SUPPLIER ***
        setTimeout(() => {
          console.log('🔄 Redirecting to supplier dashboard...');
          this.router.navigate(['/supplier/inicio']).then(() => {
            console.log('✅ Redirected to supplier dashboard successfully');
          }).catch((error: any) => {
  ...
  private createPaymentIntent(userId: number, subscriptionPlan: 'basic' | 'corporate'): Promise<PaymentIntentResponse> {
    ...
    console.log('🔍 Using selected plan:', this.selectedPlanForPayment);
