...
export class SupplierConfigurationComponent implements OnInit, OnDestroy {
  ...
  loadConfiguration(): void {
    ...
    this.configurationService.getConfigurationByUserId(userId).subscribe({
      next: (config) => {
        ...
        // DIFERENCIA: Asegurar que sea supplier en frontend
        this.configuration.userType = 'supplier';
      ...
      error: (error) => {
        ...
        // CAMBIO 5: Configuración por defecto sin batchCodeFormat - SUPPLIER
        this.configuration = new Configuration({
          userId: this.currentUserId,
          userType: 'supplier', // ← DIFERENCIA: supplier en lugar de businessman
          language: 'es',
          viewMode: 'auto',
  ...
  private savePreferencesQuietly(): void {
    ...
    this.configurationService.updateUserPreferences(
      ...
      next: (updatedConfig) => {
        ...
        this.configuration!.userType = 'supplier'; // Mantener tipo
  ...
  private savePreferencesOnly(): void {
    ...
    this.configurationService.updateUserPreferences(
      ...
      next: (updatedConfig) => {
        this.configuration = updatedConfig;
        this.configuration.userType = 'supplier'; // Mantener tipo en frontend
        this.showNotification(
          this.translateService.instant('CONFIGURATION.SUCCESS_SAVE'),
          'success'
        );
